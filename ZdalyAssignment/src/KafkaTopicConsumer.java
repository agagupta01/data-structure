			...
			try {
				...
				if (priceInvoiceResponse.getStore().equalsIgnoreCase("5363")
					...
					outputStream = new FileOutputStream(
							priceInvoiceResponse.getStore() + "_" + priceInvoiceResponse.getQuoteId() + ".txt", true);
					...
					try {
						for (int i = 0; i < priceInvoiceResponse.getRewards().size(); i++) {
							try {
								if (priceInvoiceResponse.getRewards().get(i).getPriceOverride() != null) {
								...
								LOGGER.error("Exception while processing " + i + " rewards" + e.getMessage() );
							}
						}
					} catch (Exception e) {
						LOGGER.error("getParsedEventData() ::: parsing exception for Products, error is : "
								+ e.getMessage());
